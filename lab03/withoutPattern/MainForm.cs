...
namespace BankTerminal
{
    public class MainForm : Form
    {
        ...
        private readonly List<QueueEntry> _macroBuffer = new List<QueueEntry>();
        ...
        private void InitUI()
        {
            Text = "Банковский терминал (без паттерна)";
        ...
        private void OnExecute(bool immediate)
        {
            ...
            if (type == 2)
            {
                to = cmbTo.SelectedItem as BankAccount;
                if (to == null || to == from)
            ...
            {
                OperationResult r;
                if (type == 0) r = _terminal.Deposit(from, amount);
                else if (type == 1) r = _terminal.Withdraw(from, amount);
                else r = _terminal.Transfer(from, to, amount);
                RefreshAll();
                SetStatus(r.Message, r.IsSuccess);
            }
            else
            {
                ...
                if (type == 0)
                {
                    _terminal.EnqueueDeposit(from, amount);
                    desc = string.Format("Пополнение счёта {0} на сумму {1:N2} руб.", from.Number, amount);
                }
                else if (type == 1)
                {
                    _terminal.EnqueueWithdraw(from, amount);
                    desc = string.Format("Снятие {0:N2} руб. со счёта {1}", amount, from.Number);
                }
                else
                {
                    _terminal.EnqueueTransfer(from, to, amount);
                    desc = string.Format("Перевод {0:N2} руб. {1} → {2}", amount, from.Number, to.Number);
                }
                RefreshAll();
                SetStatus("Добавлено в очередь: " + desc, true);
            }
        }
        ...
        private void OnAddToMacro(object sender, EventArgs e)
        {
            ...
            if (type == 2)
            {
                ...
                if (to == null || to == from)
                { SetStatus("Выберите другой счёт получателя.", false); return; }
                _macroBuffer.Add(new QueueEntry(OperationType.Transfer, from, to, amount));
            }
            else if (type == 0)
            {
                _macroBuffer.Add(new QueueEntry(OperationType.Deposit, from, null, amount));
            }
            else
            {
                _macroBuffer.Add(new QueueEntry(OperationType.Withdraw, from, null, amount));
            }

            RefreshMacroList();
            SetStatus("Добавлено в макрос.", true);
        }
        ...
        private void OnRunMacro(bool immediate)
        {
            ...
            var ops = new List<QueueEntry>(_macroBuffer);
            _macroBuffer.Clear();
            RefreshMacroList();

            if (immediate)
            {
                var r = _terminal.ExecuteMacro(name, ops);
                RefreshAll();
                SetStatus(r.Message, r.IsSuccess);
            }
            else
            {
                _terminal.EnqueueMacro(name, ops);
                RefreshAll();
                SetStatus(string.Format("Макрос \"{0}\" добавлен в очередь.", name), true);
            }
        }
        ...
        private void RefreshMacroList()
        {
            ...
            foreach (var op in _macroBuffer)
                lstMacroOps.Items.Add(op.Description);
        ...
        private void RefreshQueue()
        {
            ...
            foreach (var entry in _terminal.PeekQueue())
                lstQueue.Items.Add("[" + entry.TypeLabel + "]  " + entry.Description);
        }
