...
public:
    bool load(const string& path, vector<Pixel>& pixels, ImageSize& size) {
        if (path == "demo.bmp") {
            return generateDemo(pixels, size);
        }
...
public:
    PhotoProcessor() : loaded(false), filtersCount(0) {
        initPresets();
    }
    ...
    bool applyPreset(const string& name) {
        ...
        filtersCount = applyPresetInternal(it->second);
        return true;
    }
    ...
    int getFiltersCount() const { return filtersCount; }
    bool isLoaded() const { return loaded; }
    const ImageSize& getSize() const { return currentSize; }
...
private:
    ...
    int applyPresetInternal(const Preset& p) {
        int count = 0;
        for (const auto& f : p.filters) {
            filters.apply(f, pixelBuffer);
            count++;
        }
        if (p.target_colorspace != ColorSpace::RGB) {
            ColorProcessor cp;
            cp.toGrayscale(pixelBuffer);
            count++;
        }
        if (p.target_size.width > 0) {
            ImageSize target = p.target_size;
            if (target.height == 0) {
...
private:
    ...
    string jsonInfo() {
        ostringstream ss;
        ss << "{\"width\":" << processor.getSize().width
            << ",\"height\":" << processor.getSize().height
            << ",\"loaded\":" << (processor.isLoaded() ? "true" : "false")
            << ",\"filters\":" << processor.getFiltersCount() << "}";
        return ss.str();
    }
    ...
    void handlePreview(const httplib::Request&, httplib::Response& res) {
        lock_guard<mutex> lock(processorMutex);
        auto bmp = processor.getPreviewBmp();
    ...
    void handleUpload(const httplib::Request& req, httplib::Response& res) {
        ...
        out.close();
        if (processor.loadImage("temp_upload.bmp")) {
    ...
    void handlePreset(const httplib::Request& req, httplib::Response& res) {
        ...
        if (processor.applyPreset(preset)) {
            res.set_content("{\"ok\":true,\"filters\":" + to_string(processor.getFiltersCount()) + "}", "application/json");
        }
        else {
    ...
    void handleSave(const httplib::Request&, httplib::Response& res) {
        lock_guard<mutex> lock(processorMutex);
        setCors(res);
        if (processor.saveImage("output.bmp")) {
    ...
    void handleFilters(const httplib::Request&, httplib::Response& res) {
        setCors(res);
        auto f = processor.getAvailableFilters();
        string j = "[";
