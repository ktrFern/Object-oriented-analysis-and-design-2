...
class CurrencyFormatter(ABC):
    @abstractmethod
    def format(self, amount): pass

class DeliveryService(ABC):
    @abstractmethod
    def calculate(self): pass

class PromotionSystem(ABC):
    percent = 0
    @abstractmethod
    def apply(self, amount): pass
...
class RegionFactory(ABC):
    @abstractmethod
    def create_tax_policy(self) -> TaxPolicy: pass
    @abstractmethod
    def create_currency_formatter(self) -> CurrencyFormatter: pass
    @abstractmethod
    def create_delivery_service(self) -> DeliveryService: pass
    @abstractmethod
    def create_promotion_system(self) -> PromotionSystem: pass
    @abstractmethod
    def get_delivery_options(self) -> dict: pass

class GermanyTax(TaxPolicy):
    def apply(self, amount): return amount * 1.19

class EuroFormatter(CurrencyFormatter):
    def format(self, amount): return f"{amount:.2f} €"

class DHLDelivery(DeliveryService):
    def calculate(self): return 5.0

class EUPromotion(PromotionSystem):
    percent = 5
    def apply(self, amount): return amount * (self.percent / 100)

class GermanyFactory(RegionFactory):
    def create_tax_policy(self): return GermanyTax()
    def create_currency_formatter(self): return EuroFormatter()
    def create_delivery_service(self): return DHLDelivery()
    def create_promotion_system(self): return EUPromotion()
    def get_delivery_options(self): return {"DHL": 5, "Hermes": 4, "DPD": 6}

class USATax(TaxPolicy):
    def apply(self, amount): return amount * 1.07

class DollarFormatter(CurrencyFormatter):
    def format(self, amount): return f"${amount:.2f}"

class FedExDelivery(DeliveryService):
    def calculate(self): return 10.0

class USPromotion(PromotionSystem):
    percent = 10
    def apply(self, amount): return amount * (self.percent / 100)

class USAFactory(RegionFactory):
    def create_tax_policy(self): return USATax()
    def create_currency_formatter(self): return DollarFormatter()
    def create_delivery_service(self): return FedExDelivery()
    def create_promotion_system(self): return USPromotion()
    def get_delivery_options(self): return {"FedEx": 10, "UPS": 9, "USPS": 6}

class JapanTax(TaxPolicy):
    def apply(self, amount): return amount * 1.10

class YenFormatter(CurrencyFormatter):
    def format(self, amount): return f"¥{amount:.0f}"

class YuDelivery(DeliveryService):
    def calculate(self): return 800

class JapanPromotion(PromotionSystem):
    percent = 8
    def apply(self, amount): return amount * (self.percent / 100)

class JapanFactory(RegionFactory):
    def create_tax_policy(self): return JapanTax()
    def create_currency_formatter(self): return YenFormatter()
    def create_delivery_service(self): return YuDelivery()
    def create_promotion_system(self): return JapanPromotion()
    def get_delivery_options(self): return {"Yu-Pack": 800, "Kuroneko": 1200, "Sagawa": 1000}

class OrderService:
    def __init__(self, factory: RegionFactory):
        self.factory = factory
        self.tax = factory.create_tax_policy()
        self.currency = factory.create_currency_formatter()
        self.delivery = factory.create_delivery_service()
        self.promo = factory.create_promotion_system()

    def format_price(self, amount):
        return self.currency.format(amount)
...
class CheckoutWidget(QWidget):
    ...
    def refresh_region(self):
        ...
        self.delivery_options = self.order_service.factory.get_delivery_options()
        ...
        self.discount_box.setText(f"Применить скидку ({self.order_service.promo.percent}%)")
    ...
    def recalculate(self):
        ...
        discount = self.order_service.promo.apply(subtotal) if self.discount_box.isChecked() else 0
        ...
        total_with_tax = self.order_service.tax.apply(taxable_amount)
        ...
        self.total_label.setText(
            ...
            f"<div style='font-size:22px;font-weight:600;color:#1B5E20;'>{self.order_service.format_price(total)}</div>"
...
class ShopApp(QMainWindow):
    def __init__(self):
        ...
        self.products = [("Frog", 1000, "froge.png"), ("Fox", 200, "fox.png"),
                         ...
                         ("Cat", 400, "cat.png")]
        self.cart = []
        self.factories = {"🇩🇪": GermanyFactory, "🇺🇸": USAFactory, "🇯🇵": JapanFactory}
        self.current_region = "🇩🇪"
        self.order_service = OrderService(GermanyFactory())
    ...
    def change_region(self, flag):
        ...
        self.order_service = OrderService(self.factories[flag]())
